...
class Chatroom:
    ...
    def login(self):
        try:
            ...
            tk.Label(self.login_win, textvariable=self.ip_var, bg="white", font=("Noto Sans", 10)).grid()
            self.ip_var.set("IP")
            self.ip_entry = tk.Entry(self.login_win, font=("Noto Sans", 10), selectbackground="orange",
                                           highlightbackground="grey", highlightcolor="grey", relief=tk.FLAT)
            self.ip_entry.insert(0, self.ip)
            self.ip_entry.grid(sticky="we", padx=10)
...
Windows test: Windows-10-10.0.22000-SP0 with Python 3.9.6""", fg="gray", background="white", font=("Noto Mono", 8))
    ...
    def main(self):
        try:
            ...
            parser.add_argument("-i", "--ip", type=str, default="owlhosting.cloud",
